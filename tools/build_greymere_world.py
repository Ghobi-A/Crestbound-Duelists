...
def compile_world():
    ...
    for x,y,w,h in [(15,6,5,26),(12,12,11,12),(10,14,15,7),(6,10,22,2),(7,20,21,3),(8,10,3,14),(24,10,3,13)]:
        rect(grid,x,y,w,h,":")
    rect(grid,15,6,5,3,"^")
    ...
    rect(grid,3,25,3,2,"=")
    rect(grid,6,25,9,2,":")
    buildings = [prop("study",8,10),prop("guardhouse",25,10),prop("herbalist",8,21),prop("inn",25,20)]
    ...
    props += [prop("lamp",x,y) for x,y in [(11,21),(22,20),(11,10),(28,10),(15,7),(19,7),(15,28),(19,28)]]
    # Overlapping edge canopies frame every approach without blocking lanes.
    props += [prop("tree",x,y) for x,y in [(3,6),(3,12),(3,18),(3,24),(3,31),(30,6),(30,14),(30,20),(30,29),(7,32),(12,33),(23,33),(28,33),(8,3),(13,3),(22,3),(28,3)]]
    town=dict(id="greymere",name="GREYMERE TOWN",subtitle="Above the Hollow Court",scene="res://scenes/overworld/greymere.tscn",
              ...
              spawn_points=dict(approach=spawn(17,29,(0,-1)),court_return=spawn(17,7),lena_return=spawn(8,22),inn_return=spawn(25,21),silas_return=spawn(8,11)),
              doors=[entrance("lena",8,21,"lena_house","lena_return"),entrance("inn",25,20,"inn","inn_return"),entrance("silas",8,10,"silas_study","silas_return"),entrance("guard",25,10,"greymere","approach",True)],
              ...
              npcs=[npc("Warden Almyra","elara","elara_intro",19,9),npc("Liora Sen","mira","mira_intro",14,13,(1,0)),npc("Joey","townsfolk/farmboy","toby_flavor",15,20),npc("Gell","townsfolk/farmhand_capped","pell_flavor",27,23,(-1,0)),npc("Danfor","townsfolk/guard_sword","guard_flavor",24,12),npc("Watchman Orrin","townsfolk/guard_spear","watchman_flavor",20,6),npc("Old Ferris","townsfolk/torch_bearer","ferris_flavor",14,27),npc("Hooded Stranger","townsfolk/hooded_stranger","stranger_flavor",6,27)],
              lights=[dict(tile=[x,y],color="warm") for x,y in [(8,21),(25,20),(8,10),(25,10),(15,28),(19,28)]]+[dict(tile=[17,5],color="violet")])
    rooms={}
    definitions=[
      ("lena_house","LENA'S HOUSE","Herbs, hearth and home","lena_return",[prop("hearth",5,4),prop("jars",15,4),prop("workbench",5,8),prop("bed",16,8),prop("chest",15,11),prop("table",9,8)], [npc("Lena","townsfolk/herbalist_woman","wren_flavor",6,10)]),
      ("inn","GREYMERE INN","Warmth along the road","inn_return",[prop("hearth",5,4),prop("counter",14,6),prop("jars",15,3),prop("table",5,9),prop("table",14,10),prop("crates",17,12),prop("bed",17,4)], [npc("Goodwife Senna","townsfolk/elder_woman","senna_flavor",12,4),npc("Joey","townsfolk/farmboy","toby_flavor",7,10)]),
      ("silas_study","SILAS'S STUDY","Old papers. Older questions.","silas_return",[prop("books",5,4),prop("books",10,4),prop("books",15,4),prop("desk",8,8),prop("bed",17,8),prop("chest",4,11),prop("jars",15,11)], [npc("Elder Silas","townsfolk/village_elder","kassian_flavor",10,9)])]
    for id,name,subtitle,return_spawn,props,npcs in definitions:
        grid=[["_" for _ in range(22)] for _ in range(16)]
        rect(grid,1,1,20,14,"#");rect(grid,2,3,18,11,"w")
        rect(grid,10,13,2,1,":")
        rooms[id]=dict(id=id,name=name,subtitle=subtitle,scene=f"res://scenes/overworld/{id}.tscn",rows=["".join(r) for r in grid],interior=True,music_id="greymere",props=props,npcs=npcs,spawn_points={"entrance":spawn(10,12,(0,-1))},
          doors=[dict(building_id=id,tile=[10,14],destination_location="greymere",destination_scene="res://scenes/overworld/greymere.tscn",destination_spawn=return_spawn,return_spawn="entrance",locked=False,locked_dialogue_id="locked_door",transition_type="fade",verb="Leave")],
